...
| [`core/console_runner.py`](./skills/multimodal-agent-builder/core/console_runner.py) | `ConsoleAskQuestionHook`, `prompt_ask_question`, `configure_console_logging`, `add_logging_cli_args`, `configure_logging_from_args` | Implements `hooks.OnInteractionHook` for `types.BuiltinTools.ASK_QUESTION` and provides interactive/autonomous question prompting with custom write-in support. Also owns the console logging policy: third-party SDK chatter (Gen AI SDK AFC, per-request HTTP lines, Antigravity harness stderr) is suppressed unless the harness is run with `--logging`. |
